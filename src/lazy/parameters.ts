import { $ParametersObject, $ParameterObject, LazyParameter, LazyParameters } from "../model/LazyOpenApi";
import { ReferenceObject, ParameterObject } from "../model/OpenApi";
import parameter from "./parameter";

const OPEN_API_PARAMETER_IN_NAMES = new Set(["query", "header", "path", "cookie"]);
...
export default (o: $ParametersObject, path: string): (ReferenceObject | ParameterObject)[]  =>
  o instanceof Array ?
  (o as Array<ParameterObject>).map((i: ParameterObject) => parameter(i, path)) :
  Object.keys(o).filter(a => OPEN_API_PARAMETER_IN_NAMES.has(a)).length !== 0 ?
  Object.entries(o)
    .map(([k0, v0]) => Object.entries(v0)
      .map(([k1, v1]) => parameter(v1 as $ParameterObject, path, {name: k1, $in: k0})))
    .reduce((a, b) => ([...a, ...b]), []) : // { header: { ... }, query: { ... }}
