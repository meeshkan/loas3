...
test("lazy openapi preserves existing info", () => {
  ...
  expect(loai3(yaml.load(`info: { version: 0.0.1 }`)).info.version).toBe(
    "0.0.1"
  );
  expect(
    loai3(yaml.load(`info: { description: foobar }`)).info.description
  ).toBe("foobar");
  expect(
    loai3(yaml.load(`info: { version: 0.0.1, description: foobar }`)).info
      .description
  ).toBe("foobar");
});
